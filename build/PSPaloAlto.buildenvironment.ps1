...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        ModuleLicenseURI = 'https://github.com/zloeber/pspaloalto/LICENSE.md'
        ModuleTags = 'PaloAlto,Firewall,XMLAPI,REST' -split ','
        ...
        # You should NOT enter this key here but rather manually enter it in the pspaloalto.buildenvironment.json file with: set-buildenvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "pspaloalto.buildenvironment.json"
