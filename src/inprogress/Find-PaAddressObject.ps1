            ...
            foreach ($Address in $Addresses) {
                ...
                $CurrentAddress.Name = $Address.Name
                    if ($Address."ip-netmask") {
                        ...
                        if ($CurrentAddress.Value -match "/") {
                            ...
                            if ($Mask -eq 32) {
                                ...
                                if ($IpRx.Match($SearchString).Success) {
                                    #$IsFound = Test-SubnetMember "$start-$stop" $SearchString
                                    $IsFound = IsIPAddressInRange $SearchString $start $stop
                                }
                            }
                        } else {
                        ...
                        if ($IpRx.Match($SearchString).Success) {
                            $IsFound = Test-SubnetMember $CurrentAddress.value $SearchString
                        }
                    }
